...
def _joint_cog_search(px_x, px_y, px_tot, ph_x, ph_y, tol=0.005, max_states=5_000_000):
    """
    Branch-and-bound search for the pixel-to-photon assignment that reproduces
    EMPIR photon centroids exactly (inverse engineering EMPIR's clustering).

    For each group of competing photons sharing candidate pixels, finds the
    partition of those pixels into photon groups such that the TOT-weighted
    centroid of each group matches the target (ph_x[k], ph_y[k]) within `tol`.

    The balance equation per photon k: Σ_i tot_i·(x_i − X_k) = 0
    is used as the pruning criterion — remaining suffix balances bound whether
    zero is still reachable, cutting the search space dramatically.

    Parameters
    ----------
    px_x, px_y, px_tot : 1-D arrays, candidate pixel coordinates and weights
    ph_x, ph_y          : 1-D arrays, target photon centroid positions
    tol                 : acceptable centroid deviation (pixels); 0.005 → 2 d.p.
    max_states          : hard cap on recursions before giving up

    Returns
    -------
    assignment : int array length n_pixels — photon index 0..n_ph-1 or -1 (noise)
    error      : float — sum of squared centroid deviations (0 = exact match)
    """
    n_px = len(px_x)
    n_ph = len(ph_x)
    if n_px == 0 or n_ph == 0:
        return np.full(n_px, -1, dtype=int), np.inf

    # Process pixels largest-TOT first: tightest balance constraints land early
    order = np.argsort(-px_tot)
    sx = px_x[order].astype(float)
    sy = px_y[order].astype(float)
    st = px_tot[order].astype(float)

    # dx[i, k] = tot_i * (x_i - X_k): balance contribution of pixel i to photon k
    dx = (sx[:, None] - ph_x[None, :]) * st[:, None]   # (n_px, n_ph)
    dy = (sy[:, None] - ph_y[None, :]) * st[:, None]

    # Suffix min/max achievable balance for each photon k (pixel i to end)
    min_dx = np.zeros((n_px + 1, n_ph))
    max_dx = np.zeros((n_px + 1, n_ph))
    min_dy = np.zeros((n_px + 1, n_ph))
    max_dy = np.zeros((n_px + 1, n_ph))
    sfx_tot = np.zeros(n_px + 1)
    for i in range(n_px - 1, -1, -1):
        min_dx[i] = min_dx[i + 1] + np.minimum(dx[i], 0)
        max_dx[i] = max_dx[i + 1] + np.maximum(dx[i], 0)
        min_dy[i] = min_dy[i + 1] + np.minimum(dy[i], 0)
        max_dy[i] = max_dy[i + 1] + np.maximum(dy[i], 0)
        sfx_tot[i] = sfx_tot[i + 1] + st[i]

    bal_x   = np.zeros(n_ph)
    bal_y   = np.zeros(n_ph)
    sum_tot = np.zeros(n_ph)
    asgn    = np.full(n_px, -1, dtype=int)
    best    = [None, np.inf]
    states  = [0]
    ...
    def _bt(idx):
        ...
        if idx == n_px:
            ...
            for k in range(n_ph):
                if sum_tot[k] <= 0:
                    return          # every photon must own at least one pixel
                err += (bal_x[k] / sum_tot[k]) ** 2 + (bal_y[k] / sum_tot[k]) ** 2
            if err < best[1]:
                best[1] = err
                best[0] = asgn.copy()
            return

        # Pruning: for each photon k, is balance = 0 still achievable?
        rem = sfx_tot[idx]
        for k in range(n_ph):
            T_hi = sum_tot[k] + rem      # upper bound on final total TOT for k
            if T_hi <= 0:
                return
            thresh = tol * T_hi + 1e-9
            lo_x = bal_x[k] + min_dx[idx, k]
            hi_x = bal_x[k] + max_dx[idx, k]
            lo_y = bal_y[k] + min_dy[idx, k]
            hi_y = bal_y[k] + max_dy[idx, k]
            if lo_x > thresh or hi_x < -thresh or lo_y > thresh or hi_y < -thresh:
                return
    ...
    result = np.full(n_px, -1, dtype=int)
...
class Analyse:
    ...
    def _associate_pixels_to_photons_empir(self, pixels_df, photons_df,
        ...
        """
        Reverse-engineer EMPIR's pixel-to-photon grouping algorithm.

        For each photon (processed in ph/toa order):
          1. Seed  — the unclaimed pixel whose toa equals ph/toa exactly
                     (within one Timepix3 tick).  Ties broken by largest TOT.
                     Falls back to ±25 ns (coarse-clock-boundary artefact).
          ...
          3. Exact search — enumerate subsets of candidates (seed always
                     ...
                     the largest subset whose TOT-weighted L1 centroid error
                     is below COG_TOL.
          4. Relax — if not found, widen the time window and retry (up to 8×).
        """
        TICK_S       = 1.5625e-9           # Timepix3 clock tick in seconds
        COARSE_CLK_S = 16 * TICK_S         # 25 ns coarse-clock-boundary offset
        HALF_TICK    = TICK_S * 0.5
        COG_TOL      = 0.03                # L1 centroid tolerance (|Δx|+|Δy|)
        MAX_CAND     = 15                  # bitmask cap: 2^15 = 32 K subsets
        MAX_OPT_CAND = 8                   # bitmask cap for fallback: 2^8 = 256

        if pixels_df is None or photons_df is None or len(pixels_df) == 0 or len(photons_df) == 0:
            return pixels_df

        # The bit-matrix ops are small (≤32767 rows × ≤15 cols).
        # BLAS worker threads spend more time on lock overhead than computation.
        # Permanently set to 1 thread — restoring re-spins OpenBLAS (~9 s overhead).
        try:
        ...
        pix_x   = pixels['x'].to_numpy()
        pix_y   = pixels['y'].to_numpy()
        pix_tot = (pixels['tot'].to_numpy().astype(np.float32)
                   if 'tot' in pixels.columns else np.ones(len(pixels), dtype=np.float32))
        ...
        # Time window widens each step; spatial radius stays fixed.
        search_schedule = [(1, 1), (2, 1), (4, 1), (8, 1)]

        # Pre-extract photon arrays to avoid pandas row-access overhead in the loop
        ...
        def _bitmask_l1(sx, sy, st, cx_arr, cy_arr, ct_arr, nc, target_x, target_y):
            """L1 CoG error + popcount for every bitmask subset (float32, 2^nc rows)."""
            masks = np.arange(1 << nc, dtype=np.int32)
            bits  = ((masks[:, None] >> np.arange(nc, dtype=np.int32)) & 1).astype(np.float32)
            c_tot = bits @ ct_arr
            tot_s = st + c_tot
            ok    = tot_s > 0
        ...
        def _exact_subset(seed_i, cand_idx, ph_x, ph_y):
            ...
            if tot_full > 0:
                ...
                if l1_full < COG_TOL:
                    return list(range(nc)), float(l1_full)
            # Bitmask
            masks, l1, pop = _bitmask_l1(sx, sy, st, cx_arr, cy_arr, ct_arr, nc, ph_x, ph_y)
        ...
        def _optim_subset(seed_i, cand_idx, ph_x, ph_y):
            """
            Fallback: claim the subset with minimum L1 (bitmask ≤ MAX_OPT_CAND,
            full-set fast path if more candidates to avoid hangs).
            """
            sx = pix_x[seed_i]; sy = pix_y[seed_i]; st = pix_tot[seed_i]
            nc = min(len(cand_idx), MAX_CAND)
            if nc == 0:
            ...
            if nc > MAX_OPT_CAND:
                ...
                if tot_full > 0:
                    return list(range(nc)), (
                        abs((sx * st + (cx_arr * ct_arr).sum()) / tot_full - ph_x)
                      + abs((sy * st + (cy_arr * ct_arr).sum()) / tot_full - ph_y))
                return [], abs(sx - ph_x) + abs(sy - ph_y)
            masks, l1, _ = _bitmask_l1(sx, sy, st, cx_arr, cy_arr, ct_arr, nc, ph_x, ph_y)
            best = int(np.argmin(l1))
            return [i for i in range(nc) if int(masks[best]) & (1 << i)], float(l1[best])

        _print_every = max(1, n_photons // 20)
        if verbosity >= 1:
            print(f"  EMPIR pixel association ({n_photons:,} photons)  ", end='', flush=True)

        for ph_j in range(n_photons):
            if verbosity >= 1 and ph_j % _print_every == 0:
                print('.', end='', flush=True)

            ph_t  = ph_t_arr[ph_j]
            ph_x  = ph_x_arr[ph_j]
            ph_y  = ph_y_arr[ph_j]
            ph_id = ph_id_arr[ph_j]

            # ── Step 1: seed pixel ────────────────────────────────────────────
            # Use searchsorted (O(log n)) to find the narrow TOA window for each
            # offset rather than scanning all pixels.  Seed TOA offsets:
            ...
            for _toa_offset in (0, -COARSE_CLK_S, COARSE_CLK_S,
                ...
                ls = int(np.searchsorted(pix_t, t_seed - HALF_TICK))
                rs = int(np.searchsorted(pix_t, t_seed + HALF_TICK, side='right'))
                if rs <= ls:
                    continue
                sc = np.arange(ls, rs)
                sc = sc[~claimed[sc]]
            ...
            for time_mult, _ in search_schedule:
                window = base_time_s * time_mult

                # Candidates: unclaimed, toa in [seed_toa, ph_t + window], within radius
                # Use searchsorted — O(log n) instead of O(n) boolean mask over all pixels
                lc = int(np.searchsorted(pix_t, t0 - HALF_TICK))
                rc = int(np.searchsorted(pix_t, ph_t + window + HALF_TICK, side='right'))
                cand_idx = np.arange(lc, rc) if rc > lc else np.empty(0, dtype=np.intp)
                if len(cand_idx):
                    keep = ~claimed[cand_idx] & (cand_idx != seed_i)
                    cand_idx = cand_idx[keep]
                if len(cand_idx):
                    dx = pix_x[cand_idx] - ph_x
                    dy = pix_y[cand_idx] - ph_y
                    cand_idx = cand_idx[dx * dx + dy * dy <= r2_max]

                if len(cand_idx) == prev_n_cand:
                    continue
                prev_n_cand = len(cand_idx)

                # Sort by toa (time-order matching EMPIR)
                if len(cand_idx) > 1:
...
_DEFAULT_SIM_COLS = [
    ...
    'x', 'y', 'z', 'dx', 'dy', 'dz', 'toa', 'wavelength',
    'parentName', 'px', 'py', 'pz', 'parentEnergy',
    'nx', 'ny', 'nz', 'neutronEnergy',
]


def build_combined(run_dir, archive, suffix='', sim_cols=None, verbose=False):
    """
    Join AssociatedResults → TracedPhotons → SimPhotons.

    Assembles the full simulation truth chain into one pixel-level DataFrame.
    Every row carries both EMPIR reconstruction columns (px/*, ph/*, ev/*)
    and the original Geant4 truth columns (sim_id, neutron_id, photon origin,
    parent energy, etc.).
    ...
    if not sim_dir.exists():
        raise FileNotFoundError(f"SimPhotons directory not found: {sim_dir}")

    trace_files = sorted(trace_dir.glob('traced_sim_data_*.csv'))
    sim_files   = sorted(sim_dir.glob('sim_data_*.csv'))
    if not trace_files:
