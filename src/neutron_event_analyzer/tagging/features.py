...
def photon_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _features(g):
        ...
        # ToA spread
        toa_range_ns = toa.max() - toa.min()

        # bimodality: largest gap vs median gap in sorted ToA
        if npix >= 3:
            sorted_toa = np.sort(toa)
            gaps = np.diff(sorted_toa)
            max_gap = gaps.max()
            median_gap = np.median(gaps)
            toa_bimodal_ratio = max_gap / median_gap if median_gap > 0 else 0.0
        else:
        ...
        return pd.Series({
            ...
            "ph/mean_dist":         mean_dist,
            "ph/toa_range_ns":      toa_range_ns,
            "ph/toa_bimodal_ratio": toa_bimodal_ratio,
...
def event_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _features(g):
        ...
        if nph >= 2:
            dists = [
                np.sqrt((px[a] - px[b]) ** 2 + (py[a] - py[b]) ** 2)
